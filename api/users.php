        ...
        if (isset($_POST['action']) && $_POST['action'] === 'update_profile') {
            ...
            try {
                if ($avatarPath) {
                    ...
                    $sql = "UPDATE users SET username=?, email=?, specialization=?, bio=?, avatar=? WHERE id=?";
                    $stmt = $conn->prepare($sql);
                    $stmt->execute([$username, $email, $specialization, $bio, $avatarPath, $userId]);
                } else {
                    // Update tanpa ganti foto
                    $sql = "UPDATE users SET username=?, email=?, specialization=?, bio=? WHERE id=?";
                    $stmt = $conn->prepare($sql);
                    $stmt->execute([$username, $email, $specialization, $bio, $userId]);
                }
